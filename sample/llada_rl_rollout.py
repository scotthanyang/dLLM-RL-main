...
def generate_with_prefix_cache(
    ...
    for blk in range(num_blocks):
        ...
        x0, tr_idx = get_transfer_index(
            ...
            x, num_transfer[:, 0], unmask_threshold, remasking_strategy)
        x[tr_idx] = x0[tr_idx]
            ...
            if use_cache:
                if cgws is not None:
                    ...
                    x0, tr_idx = get_transfer_index(
                        ...
                        mask_blk, x[:, window_slice], num_transfer[:, i], unmask_threshold, remasking_strategy)
                    x[:, window_slice][tr_idx] = x0[tr_idx]
                else:
                    logits = model(x[:, s:], past_key_values=pkv, use_cache=True).logits
                    x0, tr_idx = get_transfer_index(
                        logits, temperature, target,
                        mask_blk, x[:, s:], num_transfer[:, i], unmask_threshold, remasking_strategy)
...
def get_transfer_index(logits, temperature, target, mask_index, x, num_transfer_tokens, threshold=None, remasking_strategy="low_confidence_dynamic"):
    ...
    if remasking_strategy in {"AR", "AR_top1"}:
        selected = torch.zeros_like(x0, dtype=torch.bool, device=x0.device)
        ...
        if has_mask.any():
            ...
            selected[rows, leftmost_idx[rows]] = True

            if remasking_strategy == "AR_top1":
                raw_top1 = torch.argmax(logits, dim=-1)
                forced_is_top1 = (
                    x0[rows, leftmost_idx[rows]]
                    == raw_top1[rows, leftmost_idx[rows]]
                )
                if threshold is not None:
                    threshold_selected = mask_index & (x0_p >= threshold)
                    threshold_selected[rows, leftmost_idx[rows]] = False
                    row_enabled = torch.zeros(x0.shape[0], dtype=torch.bool, device=x0.device)
                    row_enabled[rows] = forced_is_top1
                    threshold_selected &= row_enabled.unsqueeze(1)
...
def worker(pretrained_model, rank, prompts, orig_idx, seq_dict, step_dict, token_dict, batch_size, config):
...
if __name__ == "__main__":
    ...
    eos_token_ids = {x for x in eos_token_ids if isinstance(x, int) and x >= 0}
    pad_token_ids = {x for x in (tokenizer.pad_token_id,) if x is not None}
    eos_token_strings = [token for token in (tokenizer.eos_token, "<|endoftext|>") if token]
    rollout_metrics = [
